...
const QuizResultPage = ({ result, assignmentName, onBack, onNextLesson }) => {
    const { user } = useAuth();
    const [showReview, setShowReview] = useState(false);

    const { correctCount = 0, totalQuestions = 1, timeSpent = 0, questionHistory = [] } = result || {};
    ...
    return (
        <div className="fixed inset-0 z-50 bg-quiz-bg-dark font-quiz text-white overflow-hidden">
            ...
            <div className="relative flex flex-col min-h-screen w-full">
                ...
                <header className="w-full px-6 lg:px-20 py-4 glass-card z-10 border-b border-white/5">
                    <div className="max-w-5xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <span className="material-symbols-outlined text-[#7f13ec] text-3xl">rocket_launch</span>
                            <h2 className="text-xl font-bold tracking-tight">QUIZ<span className="text-[#7f13ec]">Fly</span></h2>
                        </div>
                        <div className="flex items-center gap-8">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 px-3 py-1 bg-green-500/10 border border-green-500/20 rounded-full">
                                    ...
                                    <span className="text-sm font-medium text-green-400">Quiz Finished</span>
                                </div>
                            </div>
                            {user?.AvatarUrl && (
                ...
                {/* Main or Review */}
                {!showReview ? (
                    <main className="flex-1 flex flex-col items-center justify-center px-6 py-6 max-w-5xl mx-auto w-full gap-8">
                        {/* Score Circle */}
                        <div className="flex flex-col items-center gap-6">
                            <div className="relative group">
                                ...
                                <div className="relative size-56 md:size-64 rounded-full flex items-center justify-center bg-quiz-bg-dark/80 backdrop-blur-xl border border-white/10 shadow-2xl">
                                    <div className="flex flex-col items-center text-center">
                                        <span className="text-4xl md:text-5xl font-bold text-white mb-1">{correctCount}/{totalQuestions}</span>
                                        <span className="text-[#7f13ec] font-bold text-base uppercase tracking-tighter">Correct</span>
                                    ...
                                    <svg className="absolute inset-0 size-full -rotate-90 pointer-events-none">
                                        <circle className="text-white/5" cx="50%" cy="50%" fill="transparent" r="46%" stroke="currentColor" strokeWidth="12"></circle>
                                        <circle
                                            className="text-[#7f13ec]" cx="50%" cy="50%" fill="transparent"
                                            pathLength="100" r="46%" stroke="currentColor"
                                            strokeDasharray="100" strokeDashoffset={strokeDashoffset}
                                            strokeLinecap="round" strokeWidth="12"
                                            style={{ transition: 'stroke-dashoffset 1.5s ease-out' }}
                                        ></circle>
                                        <circle
                                            className="text-fuchsia-500 opacity-50" cx="50%" cy="50%" fill="transparent"
                                            pathLength="100" r="46%" stroke="currentColor"
                                            strokeDasharray="100" strokeDashoffset={strokeDashoffset}
                            ...
                            <div className="text-center">
                                <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight mb-2">
                                    {percentage >= 80 ? `Xuất sắc, ${userName}!` : percentage >= 60 ? `Tốt lắm, ${userName}!` : `Cố lên, ${userName}!`}
                                </h1>
                                <p className="text-white/60 text-base">{assignmentName}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-5 w-full max-w-xl justify-center">
                            <div className="glass-card rounded-2xl p-6 flex-1 flex flex-col items-center text-center gap-3 hover:bg-white/5 transition-all">
                                <div className="size-14 rounded-xl bg-cyan-500/20 flex items-center justify-center border border-cyan-500/30">
                                    <span className="material-symbols-outlined text-cyan-400 text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>schedule</span>
                                </div>
                                <div>
                                    <p className="text-white/50 text-xs font-bold uppercase tracking-widest mb-1">Time Spent</p>
                                    <p className="text-3xl font-bold text-white">{formatTime(timeSpent)}</p>
                                </div>
                            </div>
                            <div className="glass-card rounded-2xl p-6 flex-1 flex flex-col items-center text-center gap-3 hover:bg-white/5 transition-all">
                                <div className="size-14 rounded-xl bg-fuchsia-500/20 flex items-center justify-center border border-fuchsia-500/30">
                                    <span className="material-symbols-outlined text-fuchsia-400 text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>target</span>
                                </div>
                                <div>
                                    <p className="text-white/50 text-xs font-bold uppercase tracking-widest mb-1">Accuracy</p>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-xl">
                            ...
                            >
                                <span className="material-symbols-outlined text-[#7f13ec] group-hover:rotate-12 transition-transform">visibility</span>
                                Review Answers
                            </button>
                            <button
                                ...
                                style={{ background: 'linear-gradient(to right, #7c3aed, #c026d3)', boxShadow: '0 4px 24px rgba(127,19,236,0.3)' }}
                            >
                                Next Lesson
                    ...
                    /* Review Answers View */
                    <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-6 py-6 gap-4 overflow-y-auto custom-scrollbar">
                                ...
                                {questionHistory.map((entry, idx) => (
                                    <div key={idx} className="glass-card rounded-2xl p-5 flex flex-col gap-4">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <p className="text-base font-semibold text-white leading-snug">{entry.question?.content}</p>
                                        </div>
                                        <div className="flex flex-col gap-2 pl-10">
                                            {(entry.question?.choices ?? []).map((choice) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className={`size-2.5 rounded-full shrink-0 ${isSelected ? 'bg-[#7f13ec]' : 'bg-white/20'}`}></div>
                                                        <span className={`text-sm ${isSelected ? 'text-white font-semibold' : 'text-white/60'}`}>{choice.Content}</span>
                                                        {isSelected && (
                                                            <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full bg-[#7f13ec]/30 text-[#c4b5fd]">Đã chọn</span>
                                                        )}
                                                    </div>
                                                );
                                            })}
                ...
                {/* Footer */}
                <footer className="relative z-10 w-full px-6 py-4 flex justify-center border-t border-white/5">
                    <button onClick={onBack} className="flex items-center gap-2 text-white/40 hover:text-white/60 transition-colors">
