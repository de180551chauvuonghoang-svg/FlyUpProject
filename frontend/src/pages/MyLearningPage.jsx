...
const MyLearningPage = () => {
    ...
    const inProgressEnrollments = enrollments.filter(e => calculateProgress(e) < 100);
    const completedEnrollments = enrollments.filter(e => calculateProgress(e) === 100);
            ...
            <article key={enrollment.CourseId} className="group relative flex flex-col bg-surface-dark border border-white/5 rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-neon hover:-translate-y-1">
                ...
                >
                    ...
                    <div className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/10 flex items-center gap-1.5">
                        <span className={`size-2 rounded-full ${isCompleted ? 'bg-sky-500' : 'bg-emerald-500 animate-pulse'}`}></span>
                        <span className="text-[10px] font-bold text-white uppercase tracking-wider">{isCompleted ? 'Completed' : 'Active'}</span>
                    </div>
                </div>
                <div className="flex flex-col flex-1 p-5 gap-4">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs font-medium">
                            ...
                            <span className="text-slate-500">{isCompleted ? 'Finished' : 'Keep going'}</span>
                        </div>
                    ...
                    <div className="mt-auto pt-2">
                        <button 
                            ...
                            className={`w-full py-2.5 rounded-full ${isCompleted ? 'bg-white/10 hover:bg-white/20' : 'bg-primary hover:bg-primary/90'} text-white font-semibold text-sm transition-all shadow-[0_0_15px_rgba(110,60,236,0.4)] hover:shadow-[0_0_20px_rgba(110,60,236,0.6)] flex items-center justify-center gap-2`}
                        >
                ...
                <div className="w-full max-w-[1280px] flex flex-col gap-8">
                    ...
                    <div className="border-b border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-0">
                        <div className="flex gap-8 overflow-x-auto w-full sm:w-auto no-scrollbar">
                            ...
                            >
                                <span className="flex items-center gap-2">Completed <span className="bg-primary/20 text-primary text-[10px] px-1.5 py-0.5 rounded-full">{completedCount}</span></span>
                    ...
                    {activeTab === 'in_progress' && (
                        inProgressEnrollments.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {inProgressEnrollments.map(renderEnrollmentCard)}
                            </div>
                        ) : (
