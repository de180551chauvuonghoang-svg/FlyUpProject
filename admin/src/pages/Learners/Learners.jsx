...
/**
 * Learners Page
 * Learner management with list, search, pagination, lock/unlock
 */
function Learners() {
    ...
    const handleLockUser = async (userId) => {
        if (!window.confirm('Are you sure you want to lock this user?')) return;
        try {
    ...
    const handleUnlockUser = async (userId) => {
        if (!window.confirm('Are you sure you want to unlock this user?')) return;
        try {
            setActionLoading(userId);
    ...
    const renderTableContent = () => {
        ...
        return users.map((user, index) => (
            <motion.tr
                ...
                onClick={() => navigate(`/learners/${user.id}`)}
            ...
            <header className="page-header">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                    <h1 className="page-title inline-title">
                        <span className="title-accent">Learner</span>{' '}
