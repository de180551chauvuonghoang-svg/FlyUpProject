...
import { generateAdaptiveQuiz } from '../services/ai/adaptive-quiz-curation-service.js';
import { createQuizAssignment, getPrimarySectionId } from '../utils/quiz-assignment-factory.js';
  ...
  try {
    ...
    return res.status(200).json({
      ...
      questions: quiz.questions.map(q => ({
        ...
        sectionId: q.assignment.SectionId,
        sectionTitle: q.assignment.Section.Title,
      })),
...
export const getQuizGenerationHealth = async (req, res) => {
  try {
    ...
    try {
      const { default: redisClient } = await import('../lib/redis.js');
