...
} from './aiPromptBuilderUtilities.js';
...
export async function getPersonalizedRecommendations(userId, limit = 5) {
  try {
    ...
    try {
      ...
      console.error(`[Recommendations] AI call failed:`, aiError);
      console.warn(`[Recommendations] Falling back to rule-based logic`);
...
/**
 * Generate AI-powered recommendations using Vercel AI SDK
 * @private
 */
    ...
    output: Output.object({
      schema: z.object({
        recommendations: z.array(z.object({
          ...
          score: z.number().min(0).max(1).describe('The confidence score from 0 to 1'),
          reasoning: z.string().describe('Short explanation why this course matches the user')
        }))
      })
    }),
    system: 'You are an expert educational advisor. Based on the user data, recommend the best courses. Return the response in JSON format.',
    prompt: prompt,
  });
