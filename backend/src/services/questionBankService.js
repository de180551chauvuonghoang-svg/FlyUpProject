    ...
    const createdQuestion = await prisma.$transaction(async (tx) => {
        const question = await tx.questionBankQuestions.create({
            data: {
                ...
                ParamA: (paramA === '' || paramA === null || paramA === undefined) ? 1.0 : Number(paramA),
                ParamB: (paramB === '' || paramB === null || paramB === undefined) ? (difficulty === 'Easy' ? -1.0 : difficulty === 'Hard' ? 1.0 : 0.0) : Number(paramB),
                ParamC: (paramC === '' || paramC === null || paramC === undefined) ? 0.25 : Number(paramC),
